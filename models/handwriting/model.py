...
from base.contracts import ModelOutput, ModalityResult, SHAPFeature
...
class DenseNet201BinarySpec:
    ...
    input_size: int = 224
    class_names: dict[int, str] = field(default_factory=lambda: {0: "Healthy", 1: "Parkinson"})


def _build_architecture(input_size: int = 224) -> Model:
    base_model = DenseNet201(weights="imagenet", include_top=False, input_shape=(input_size, input_size, 3))
...
class DenseNet201BinaryModel:
    ...
    def VALIDATION_AUC(self) -> float:
        return float(self.model_meta.get("best_val_acc", 1.0))
    ...
    def _load_model(self) -> tuple[Model, dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Unable to load handwriting model artifact: {artifact_path}") from exc
            model_meta = {}
    ...
    def _resolve_image(self, input_data: Any) -> np.ndarray:
        ...
        if isinstance(input_data, (str, Path)):
            ...
            if path.is_dir():
                image_files = sorted(
                    candidate for candidate in path.iterdir() if candidate.is_file() and candidate.suffix.lower() in {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff"}
                )
                if not image_files:
                    raise FileNotFoundError(f"No image files found in handwriting directory: {path}")
        ...
        raise ValueError(f"Handwriting input must be an image path or 2D/3D array, got shape {getattr(array, 'shape', None)}")
    ...
    def _preprocess(self, image: np.ndarray) -> tf.Tensor:
        ...
        if tensor.shape.rank != 3:
            raise ValueError(f"Expected image tensor rank 2 or 3, got shape {tensor.shape}")

        if tensor.shape[-1] == 1:
            tensor = tf.image.grayscale_to_rgb(tensor)
        elif tensor.shape[-1] != 3:
            raise ValueError(f"Handwriting image must have 1 or 3 channels, got shape {tensor.shape}")

        tensor = tf.image.resize(tensor, (self.spec.input_size, self.spec.input_size), method="bilinear")
        tensor = tensor / 255.0
        return tensor
    ...
    def predict(self, input_data) -> ModelOutput:
        ...
        return ModelOutput(
            ...
            metadata={
                ...
                "input_source": self.spec.modality,
            },
        )


class InferenceFuser:
    def __init__(self, modality: str, models: list[DenseNet201BinaryModel], weights: dict[str, float]):
        self.modality = modality
        self.models = models

        total = sum(weights.values())
        if total <= 0:
            raise ValueError(f"InferenceFuser({modality}): weight values must sum > 0")
    ...
    def fuse_one(self, input_data) -> ModalityResult:
        ...
        raw_weights = np.array([self.weights.get(output.model_id, 1.0) for output in outputs], dtype=float)
        used_weights = raw_weights / raw_weights.sum() if raw_weights.sum() > 0 else np.ones(len(outputs), dtype=float) / len(outputs)
        ...
        ci_low = float(np.percentile(probabilities, 2.5)) if len(probabilities) > 1 else fused_probability
        ci_high = float(np.percentile(probabilities, 97.5)) if len(probabilities) > 1 else fused_probability
        ...
        return ModalityResult(
            ...
            metadata={
                ...
                "ensemble_weights": {output.model_id: float(weight) for output, weight in zip(outputs, used_weights)},
            },
        )
...
def predict_handwriting_batch(samples):
    ...
    return ModalityResult(
        ...
        metadata={
            ...
            "per_sample_probabilities": [round(float(probability), 4) for probability in probabilities],
        },
    )
